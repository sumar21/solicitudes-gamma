...
export default function App() {
  ...
  // Helper to check if user is Read Only
  const isReadOnly = state.currentUser?.role === Role.READ_ONLY || 
                     state.currentUser?.role === Role.NURSING || 
                     state.currentUser?.role === Role.HOUSEKEEPING ||
                     state.currentUser?.role === Role.COORDINATOR; // Assuming Coordinator is also read-only for operations based on prompt "Resto de grupos... solo lectura"

  // Actually, prompt says: "solo admision / admin y azafata" have access to operations.
  const hasOperationalAccess = state.currentUser?.role === Role.ADMISSION || 
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-100 p-4 md:p-6">
        <Card className="w-full max-w-md p-6 md:p-10 shadow-xl rounded-3xl border-none bg-white">
          ...
          <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-slate-100 flex flex-wrap justify-between gap-2">
             <button onClick={() => actions.setLoginEmail('adm@ig.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Demo IG</button>
             <button onClick={() => actions.setLoginEmail('adm@hpr.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Demo HPR</button>
             <button onClick={() => actions.setLoginEmail('admision@hpr.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Admisión</button>
             <button onClick={() => actions.setLoginEmail('azafata4@hpr.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Azafata P4</button>
             <button onClick={() => actions.setLoginEmail('mucama@hpr.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Mucama</button>
             <button onClick={() => actions.setLoginEmail('coord.enf@hpr.com')} className="text-[9px] font-bold text-slate-400 hover:text-zinc-950 uppercase">Coord. Enf</button>
          </div>
        </Card>
      </div>
    );
    ...
    <div className="h-screen w-full flex flex-col md:flex-row bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative pb-16 md:pb-0">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50/50">
            {hasOperationalAccess && state.currentView === 'HOME' && <DashboardView tickets={state.filteredTickets} />}
            ...
            {hasOperationalAccess && state.currentView === 'HISTORY' && <HistoryView tickets={state.filteredTickets} />}
            {/* Show BEDS view if selected OR if user is read-only (fallback) */}
            {(state.currentView === 'BEDS' || !hasOperationalAccess) && <BedsView beds={state.beds} />}
        </main>
      </div>
