...
| **Un dispositivo dejó de recibir push tras días sin abrirse** | La sub quedó **stale** (`last_seen_at` > 36h). El heartbeat (`touchPushSubscription`, cada ~6h con la app abierta) la mantiene viva | Esperado. Al reabrir la PWA se refresca el `last_seen_at` y vuelve a entrar en el envío |
